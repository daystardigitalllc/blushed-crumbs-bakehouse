...
<div id="menu-page-view">
    <section class="menu-hero-section">
        ...
        <p style="font-size:1.02rem; color:var(--dark-text); opacity:0.8; max-width:650px; margin:0 auto 24px auto;">Explore our handcrafted baked goods and custom options.</p>
        <button onclick="openOrderModal()" class="btn btn-primary" style="padding:12px 32px; font-size:1rem;">
    ...
    @else
        ...
        @if(($menuType === 'image' || $menuType === 'both') && $hasImage)
            <section class="image-menu-wrapper">
                <h2 style="font-family:var(--theme-heading-font); color:var(--dark-text); margin-bottom:12px; font-size:1.9rem;">Official Menu</h2>
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(0,0,0,0.1); border-radius:20px; padding:40px 20px; box-shadow:0 10px 30px rgba(0,0,0,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined petal-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
        ...
        @if($hasProducts && ($menuType === 'text' || $menuType === 'both'))
            <section style="max-width:1100px; margin:40px auto 40px auto; padding:0 20px;">
                <h2 style="font-family:var(--theme-heading-font); color:var(--dark-text); text-align:center; margin-bottom:25px; font-size:2.1rem;">Featured Catalog &amp; Pricing</h2>

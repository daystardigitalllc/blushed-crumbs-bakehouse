...
<head>
    ...
    <style>
        .menu-hero-section {
            background: var(--theme-section-bg, var(--pink-bg, #eef2ff));
            text-align: center;
...
<div id="menu-page-view">
    ...
    @if(!$hasMenuContent)
        ...
        <section style="max-width:700px; margin:70px auto; padding:60px 30px; text-align:center; background:var(--theme-card-bg, #ffffff); border-radius:24px; border:2px dashed #ec4899; box-shadow:0 10px 30px rgba(0,0,0,0.04);">
            <div class="material-symbols-outlined modern-icon" style="font-size:3.5rem; margin-bottom:16px;">bakery_dining</div>
            ...
            <section class="image-menu-wrapper">
                ...
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(0,0,0,0.1); border-radius:20px; padding:40px 20px; box-shadow:0 10px 30px rgba(0,0,0,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined modern-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    @foreach($products as $p)
                        <div class="category-card-exact" style="padding:22px; display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            ...
                            <div style="font-size:1.3rem; font-weight:700; color:#ec4899;">${{ number_format($p->price, 2) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>

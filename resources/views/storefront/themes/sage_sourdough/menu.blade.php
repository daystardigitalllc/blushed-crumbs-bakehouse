...
<head>
    ...
    <style>
        ...
        .sg-menu-hero-subtitle {
            text-transform: uppercase;
            font-size: 0.85rem;
            letter-spacing: 3px;
            font-weight: 700;
            color: var(--sg-forest, #334437);
            margin-bottom: 12px;
            display: block;
        }
        .sg-menu-hero-title {
        ...
        .sg-image-menu-wrapper {
            max-width: 900px;
            margin: 40px auto;
            padding: 20px;
            text-align: center;
        }
        .sg-image-menu-wrapper img {
            width: 100%;
            max-width: 850px;
            border-radius: 16px;
            box-shadow: 0 15px 40px rgba(51,68,55,0.12);
            border: 1px solid rgba(0,0,0,0.08);
            cursor: pointer;
        }
        .sg-menu-text-section {
            max-width: 950px;
            ...
            border-radius: 20px;
            border: 1px solid rgba(0,0,0,0.08);
            box-shadow: 0 10px 30px rgba(51,68,55,0.05);
        }
        .sg-menu-text-section h2,
        .sg-menu-text-section h3 {
            font-family: var(--theme-heading-font, 'Fraunces', serif);
            color: var(--sg-forest, #334437);
            margin-top: 32px;
            margin-bottom: 16px;
            padding-bottom: 10px;
            border-bottom: 2px dashed var(--sg-gold, #e8c079);
            font-size: 1.6rem;
            font-weight: 700;
        }
        .sg-menu-text-section h2:first-child,
        .sg-menu-text-section h3:first-child {
            margin-top: 0;
        }
        .sg-menu-text-section p {
            font-size: 1.05rem;
            line-height: 1.8;
            color: var(--sg-forest, #334437);
            margin-bottom: 16px;
        }
        .sg-menu-text-section ul,
        .sg-menu-text-section ol {
            margin: 14px 0 24px 0;
            padding-left: 10px;
            list-style-type: none;
        }
        .sg-menu-text-section li {
            position: relative;
            padding-left: 26px;
            margin-bottom: 10px;
            font-size: 1.02rem;
        ...
        .sg-menu-text-section ul li::before {
            content: "🌿";
            position: absolute;
            left: 0;
            top: 0;
...
<div id="menu-page-view">
    <section class="sg-menu-hero-section">
        <span class="sg-menu-hero-subtitle">OUR OFFERINGS</span>
        <h1 class="sg-menu-hero-title">Menu &amp; Pricing</h1>
        <p style="font-size:1.05rem; color:var(--sg-forest); opacity:0.85; max-width:650px; margin:0 auto 24px auto;">Explore our handcrafted baked goods and custom options.</p>
        <button onclick="openOrderModal()" class="btn btn-primary" style="padding:12px 32px; font-size:0.95rem; border-radius:8px;">
            Order Custom Creation <span class="material-symbols-outlined" style="font-size:1.1rem; vertical-align:-3px;">bakery_dining</span>
        </button>
    </section>
    ...
    @if(!$hasMenuContent)
        ...
        <section style="max-width:700px; margin:70px auto; padding:60px 30px; text-align:center; background:var(--theme-card-bg, #ffffff); border-radius:20px; border:2px dashed var(--sg-gold, #e8c079); box-shadow:0 10px 30px rgba(51,68,55,0.05);">
            <div class="material-symbols-outlined sg-icon" style="font-size:3.5rem; margin-bottom:16px;">bakery_dining</div>
            <h2 style="font-family:var(--theme-heading-font); color:var(--sg-forest); font-size:1.8rem; margin-bottom:12px;">Menu Coming Soon</h2>
            ...
            <button onclick="openOrderModal()" class="btn btn-primary" style="padding:12px 30px; font-size:0.9rem; border-radius:8px;">
                Request Custom Order Quote <span class="material-symbols-outlined" style="font-size:1rem; vertical-align:-2px;">bakery_dining</span>
            </button>
        </section>
    @else
        ...
        @if(($menuType === 'image' || $menuType === 'both') && $hasImage)
            <section class="sg-image-menu-wrapper">
                <h2 style="font-family:var(--theme-heading-font); color:var(--sg-forest); margin-bottom:12px; font-size:1.8rem;"><span class="material-symbols-outlined" style="vertical-align:-4px;">description</span> Official Bakery Menu</h2>
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(0,0,0,0.1); border-radius:20px; padding:40px 20px; box-shadow:0 10px 30px rgba(51,68,55,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined sg-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
                        <h3 style="font-family:var(--theme-heading-font); color:var(--sg-forest); margin-bottom:8px;">Official Menu PDF</h3>
                        <p style="color:#666; font-size:0.95rem; margin-bottom:20px;">Click below to view or download our full menu PDF</p>
                        <a href="{{ asset($imagePath) }}" target="_blank" class="btn btn-primary" style="padding:12px 28px; font-size:0.9rem; border-radius:8px; display:inline-block; text-decoration:none;">
        ...
        @if($hasProducts && ($menuType === 'text' || $menuType === 'both'))
            <section style="max-width:1100px; margin:40px auto 40px auto; padding:0 20px;">
                <h2 style="font-family:var(--theme-heading-font); color:var(--sg-forest); text-align:center; margin-bottom:25px; font-size:2rem;"><span class="material-symbols-outlined" style="vertical-align:-5px;">bakery_dining</span> Featured Catalog &amp; Pricing</h2>
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    @foreach($products as $p)
                        <div class="category-card-exact" style="padding:22px; background:var(--theme-card-bg, #ffffff); border-radius:16px; box-shadow:0 6px 20px rgba(51,68,55,0.05); display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            <div>
                                <h4 style="margin:0 0 4px 0; font-size:1.1rem; color:var(--sg-forest);">{{ $p->name }}</h4>
                                <span style="font-size:0.85rem; color:#777;">Base Price</span>
                            </div>
                            <div style="font-size:1.3rem; font-weight:700; color:var(--sg-forest);">${{ number_format($p->price, 2) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>
        ...
        @if(!empty($customText))
            <section class="sg-menu-text-section">

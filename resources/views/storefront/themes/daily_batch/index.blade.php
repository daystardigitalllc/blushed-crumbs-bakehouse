...
<div id="storefront-view">
    ...
    @foreach($sections as $secId => $sec)
        @if(!empty($sec['enabled']))
            @if($secId === 'hero')
                <!-- Hero — solid mustard band, big bold headline, offset circular photo -->
                @php
                    ...
                    <div class="db-hero-inner">
                        <div class="db-hero-copy">
                            ...
                            <div class="hero-buttons">
                                <button onclick="openOrderModal()" class="btn btn-primary">{{ $tenant->getSiteContent('hero_cta_primary', 'Discover') }}</button>
                        ...
                        <div class="db-hero-media">
                            ...
                            @else
                                <div class="db-hero-media-placeholder"><span class="material-symbols-outlined" style="font-size:5rem;">bakery_dining</span></div>
                            @endif
                        </div>
                    </div>
                </section>
            @elseif($secId === 'about')
                <!-- About / Our Story — circular photo left, bio right -->
                <section class="db-section db-band-white">
                    <div class="db-about-row">
                        <div class="db-about-circle">
                            ...
                            @else
                                <div class="db-circle-placeholder"><span class="material-symbols-outlined" style="font-size:3rem;">cake</span></div>
                            @endif
                        </div>
                        <div class="db-about-copy">
                            ...
                            <a href="{{ route('storefront.about') }}" class="db-text-link">Read our full story →</a>
                        </div>
                    </div>
                </section>
            @elseif($secId === 'highlights')
                <!-- Highlights — mirrored row: copy + bullet list left, circular photo right -->
                <section class="db-section db-band-white">
                    <div class="db-about-row db-about-row-reverse">
                        <div class="db-about-circle">
                            ...
                            @else
                                <div class="db-circle-placeholder"><span class="material-symbols-outlined" style="font-size:3rem;">auto_awesome</span></div>
                            @endif
                        </div>
                        <div class="db-about-copy">
                            <h2>Why {{ $tenant->name }}</h2>
                            <ul class="db-highlight-list">
                                @php $highlights = $tenant->getSiteContent('highlights', []); @endphp
                                @foreach($highlights as $hl)
                                    <li><span class="db-highlight-emoji">{{ $hl['icon'] ?? '🎂' }}</span> <strong>{{ $hl['title'] ?? '' }}</strong> — {{ $hl['desc'] ?? '' }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
            ...
            @elseif($secId === 'promo_video')
                ...
                @endphp
                @if(!empty($promoVid))
                    <section class="video-promo-banner db-promo-banner">
                        <video autoplay loop muted playsinline>
                ...
                @else
                    <section class="video-promo-banner db-promo-banner" style="position:relative; background: {{ !empty($promoBg) ? 'linear-gradient(135deg, rgba(20,20,15,0.78) 0%, rgba(20,20,15,0.55) 100%), url(' . asset($promoBg) . ') center/cover no-repeat' : 'linear-gradient(135deg, #14140f 0%, #3a3a2f 100%)' }}; padding: 75px 25px; text-align: center;">
                        <div class="video-overlay-content" style="position:relative; z-index:2; max-width:720px; margin:0 auto;">
                            <span class="material-symbols-outlined" style="font-size:2.2rem; display:block; margin-bottom:10px; color:#f2d310;">redeem</span>
                            ...
                            <p style="font-size:1.05rem; color:rgba(255,255,255,0.85); margin-bottom:24px;">{{ $tenant->getSiteContent('promo_subtext', 'Order online directly from our kitchen for your upcoming celebration.') }}</p>
            ...
            @elseif($secId === 'categories')
                <!-- Categories — circular shelf row -->
                <section id="categories" class="db-section db-band-mustard">
                    <h2 class="db-section-title">Our Categories</h2>
                    <div class="db-shelf-row">
                        @php
                        ...
                        @foreach($catList as $idx => $cat)
                            ...
                            @endphp
                            <div class="db-shelf-card">
                                <div class="db-shelf-frame">
                                    @if($imgUrl)
                                        <img src="{{ $imgUrl }}" alt="{{ $cat['title'] ?? ($tenant->name . ' Bakery Category') }}" loading="lazy" decoding="async">
                                    @else
                                        <span class="material-symbols-outlined db-icon">cake</span>
                                    @endif
                                </div>
            ...
            @elseif($secId === 'whimsical')
                <!-- Whimsical Section -->
                <section class="whimsical-section db-whimsical">
                    <div class="whimsical-two-column">
                        <div class="whimsical-col-left">
                            ...
                            @endphp
                            @if($wImg)
                                <div class="db-whimsical-img">
                                    <img src="{{ asset($wImg) }}" alt="{{ $tenant->name }} Whimsical Creation" loading="lazy" decoding="async">
                                </div>
                            @else
                                <div style="text-align:center; padding:40px 20px; background:rgba(255,255,255,0.08); border-radius:24px;">
                                    <span class="material-symbols-outlined" style="font-size:4rem; display:block; margin-bottom:12px; color:#f2d310;">auto_awesome</span>
                    ...
                    <div class="db-steps-list">
                        ...
                        @foreach($steps as $idx => $step)
                            <div class="db-step-row">
                                <span class="db-step-num"></span>
                                <div class="db-step-copy">
                                    <h3>{{ $step['title'] ?? '' }}</h3>
                                    <p>{{ $step['desc'] ?? '' }}</p>
                                </div>
                            </div>
            ...
            @elseif($secId === 'reviews')
                ...
                <!-- Reviews -->
                <section id="reviews" class="reviews-section db-section db-band-gray">
                    <h2 class="db-section-title">Sweet Words from Our Customers</h2>
                    <div class="db-review-row">
            ...
            @elseif($secId === 'faq')
                <!-- FAQ & Bakery Policies -->
                <section class="faq-policies-section db-section db-band-white">
                    <h2 class="db-section-title" style="text-align:center; margin-bottom:15px;">Frequently Asked Questions</h2>
            ...
            @elseif($secId === 'featured_gallery')
                <!-- Featured Photos Gallery — "Donuts/Treats of the Week" pink-backed card grid -->
                @php $featuredImages = $tenant->getSiteContent('featured_gallery_images', []); @endphp
                @if(!empty($featuredImages))
                    <section class="featured-gallery-section db-section db-band-gray">
                        <h2 class="db-section-title" style="text-align:center;">{{ $tenant->getSiteContent('featured_gallery_title', 'Featured Creations') }}</h2>
                        <div class="gallery-masonry-grid db-featured-grid" style="max-width:1150px; margin:0 auto;">
            ...
            @elseif($secId === 'cta_banner')
                <!-- Footer Call to Action Banner — solid black -->
                @php
                    $ctaBg = $tenant->getSiteContent('cta_banner_url') ?: $tenant->getSiteContent('cta_bg_image_url');
                @endphp
                <section class="cta-video-banner db-cta-banner" style="position:relative; background: {{ !empty($ctaBg) ? 'linear-gradient(135deg, rgba(20,20,15,0.82) 0%, rgba(20,20,15,0.6) 100%), url(' . asset($ctaBg) . ') center/cover no-repeat' : '#14140f' }}; padding: 70px 25px; text-align: center;">
                    <div class="cta-content" style="max-width:750px; margin:0 auto; position:relative; z-index:2;">
                        <h2 style="font-size: 2.6rem; color: #f2d310; margin-bottom: 10px;">{{ $tenant->getSiteContent('cta_headline', 'Ready For Your Perfect Cake?') }}</h2>
                        <p style="font-size: 1.1rem; color: #ffffff; opacity: 0.9; margin-bottom: 24px;">{{ $tenant->getSiteContent('cta_subtext', 'Order your plan or custom order now') }}</p>

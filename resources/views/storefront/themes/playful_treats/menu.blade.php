...
<head>
    ...
    <style>
        ...
        .menu-text-section h3 {
            ...
            margin-bottom: 16px;
            padding-bottom: 10px;
            border-bottom: 2px dashed var(--primary, #FF4FA0);
            font-size: 1.6rem;
            font-weight: 700;
        }
...
<div id="menu-page-view">
    ...
    @if(!$hasMenuContent)
        ...
        <section style="max-width:700px; margin:70px auto; padding:60px 30px; text-align:center; background:var(--theme-card-bg, #ffffff); border-radius:24px; border:2px dashed var(--primary, #FF4FA0); box-shadow:0 10px 30px rgba(0,0,0,0.04);">
            <div class="material-symbols-outlined playful-icon" style="font-size:3.5rem; margin-bottom:16px;">bakery_dining</div>
            ...
            <section class="image-menu-wrapper">
                ...
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(0,0,0,0.1); border-radius:20px; padding:40px 20px; box-shadow:0 10px 30px rgba(0,0,0,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined playful-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    @foreach($products as $p)
                        <div class="category-card-exact" style="padding:22px; background:var(--theme-card-bg, #ffffff); border-radius:18px; box-shadow:0 6px 20px rgba(0,0,0,0.05); display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            <div>
                                <h4 style="margin:0 0 4px 0; font-size:1.1rem; color:var(--dark-text);">{{ $p->name }}</h4>
                                <span style="font-size:0.85rem; color:#777;">Base Price</span>
                            </div>
                            <div style="font-size:1.3rem; font-weight:700; color:var(--primary, #FF4FA0);">${{ number_format($p->price, 2) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>

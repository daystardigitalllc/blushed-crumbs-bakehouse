...
<head>
    ...
    <style>
        ...
        .menu-text-section h3 {
            font-family: var(--theme-heading-font, 'Playfair Display', serif);
            color: var(--dark-text, #14304a);
            margin-top: 32px;
            margin-bottom: 16px;
            padding-bottom: 10px;
            border-bottom: 2px dashed #b8935a;
            font-size: 1.6rem;
            font-weight: 700;
        }
    ...
    @else
        ...
        @if(($menuType === 'image' || $menuType === 'both') && $hasImage)
            <section class="image-menu-wrapper">
                <h2 style="font-family:var(--theme-heading-font); color:var(--dark-text); margin-bottom:12px; font-size:1.8rem;">Official Menu</h2>
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(0,0,0,0.1); border-radius:4px; padding:40px 20px; box-shadow:0 10px 30px rgba(0,0,0,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined midnight-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    @foreach($products as $p)
                        <div class="category-card-exact" style="padding:22px; display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            <div>
                                <h4 style="margin:0 0 4px 0; font-size:1.1rem; color:var(--dark-text); font-family:'Playfair Display',serif;">{{ $p->name }}</h4>
                                <span style="font-size:0.85rem; color:#777;">Base Price</span>
                            </div>
                            <div style="font-size:1.3rem; font-weight:700; color:#b8935a;">${{ number_format($p->price, 2) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>

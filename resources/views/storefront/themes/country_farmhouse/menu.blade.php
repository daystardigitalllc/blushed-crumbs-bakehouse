...
<head>
    ...
    <style>
        .menu-hero-section {
            background: var(--theme-section-bg, var(--pink-bg, #f6efe1));
            text-align: center;
        ...
        .menu-hero-title {
            font-family: var(--theme-heading-font, 'Oswald', sans-serif);
            text-transform: uppercase;
            font-size: 3.5rem;
            font-weight: 700;
            color: var(--dark-text, #2b2117);
            line-height: 1.15;
            max-width: 800px;
        ...
        .menu-text-section h3 {
            ...
            margin-bottom: 16px;
            padding-bottom: 10px;
            border-bottom: 2px dashed #c1810a;
            font-size: 1.6rem;
            font-weight: 700;
        }
...
<div id="menu-page-view">
    <section class="menu-hero-section">
        ...
        <p style="font-size:1.05rem; color:var(--dark-text); opacity:0.85; max-width:650px; margin:0 auto 24px auto;">{{ $tenant->getSiteContent('menu_hero_text', 'Explore our handcrafted baked goods and custom options.') }}</p>
        <button onclick="openOrderModal()" class="btn btn-primary" style="padding:12px 32px; font-size:1.05rem;">
            Order Custom Creation <span class="material-symbols-outlined" style="font-size:1.1rem; vertical-align:-3px;">shopping_bag</span>
        </button>
    </section>
    ...
    @if(!$hasMenuContent)
        ...
        <section style="max-width:700px; margin:70px auto; padding:60px 30px; text-align:center; background:var(--theme-card-bg, #ffffff); border-radius:6px; border:2px dashed #c1810a; box-shadow:0 10px 30px rgba(0,0,0,0.04);">
            <div class="material-symbols-outlined farmhouse-icon" style="font-size:3.5rem; margin-bottom:16px;">restaurant</div>
            ...
            <button onclick="openOrderModal()" class="btn btn-primary" style="padding:12px 30px; font-size:1rem;">
                Request Custom Order Quote <span class="material-symbols-outlined" style="font-size:1rem; vertical-align:-2px;">shopping_bag</span>
            </button>
        </section>
    @else
        ...
        @if(($menuType === 'image' || $menuType === 'both') && $hasImage)
            <section class="image-menu-wrapper">
                <h2 style="font-family:var(--theme-heading-font); text-transform:uppercase; color:var(--dark-text); margin-bottom:12px; font-size:1.8rem;"><span class="material-symbols-outlined" style="vertical-align:-4px;">description</span> Official Menu</h2>
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(0,0,0,0.1); border-radius:6px; padding:40px 20px; box-shadow:0 10px 30px rgba(0,0,0,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined farmhouse-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
                        ...
                        <a href="{{ asset($imagePath) }}" target="_blank" class="btn btn-primary" style="padding:12px 28px; font-size:1rem; display:inline-block; text-decoration:none;">
        ...
        @if($hasProducts && ($menuType === 'text' || $menuType === 'both'))
            <section style="max-width:1100px; margin:40px auto 40px auto; padding:0 20px;">
                <h2 style="font-family:var(--theme-heading-font); text-transform:uppercase; color:var(--dark-text); text-align:center; margin-bottom:25px; font-size:2rem;"><span class="material-symbols-outlined" style="vertical-align:-5px;">restaurant</span> Featured Catalog &amp; Pricing</h2>
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    @foreach($products as $p)
                        <div class="category-card-exact" style="padding:22px; display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            <div>
                                <h4 style="margin:0 0 4px 0; font-size:1.1rem; color:var(--dark-text); text-transform:uppercase;">{{ $p->name }}</h4>
                                <span style="font-size:0.85rem; color:#777;">Base Price</span>
                            </div>
                            <div style="font-size:1.3rem; font-weight:700; color:#c1810a;">${{ number_format($p->price, 2) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>

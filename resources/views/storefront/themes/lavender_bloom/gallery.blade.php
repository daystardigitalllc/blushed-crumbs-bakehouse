...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@500;600;700;800&family=Poppins:wght@400;500;600;700&family=Great+Vibes&display=swap" rel="stylesheet">
...
<div id="gallery-page-view">
    <section class="lb-page-hero">
        <span class="lb-page-hero-kicker">Follow Along</span>
        <h1 class="lb-page-hero-title">{{ $tenant->getSiteContent('gallery_hero_title', 'Our Gallery') }}</h1>
        <p style="font-size:1.05rem; color:var(--lb-ink); max-width:600px; margin:14px auto 0 auto; padding:0 15px;">{{ $tenant->getSiteContent('gallery_hero_text', 'Explore our latest custom creations uploaded directly from our kitchen!') }}</p>
    </section>
    ...
    <section class="gallery-page-section" style="padding-top:0;">
        ...
        <!-- gallery-masonry-grid is kept (not renamed) since app.js's filter
             and add-photo logic selects cards by that class name; the purple
             photo-backdrop treatment is layered on via lb-featured-grid. -->
        <div class="gallery-masonry-grid lb-featured-grid" id="public-gallery-grid">
            ...
            @empty
                <div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: #888;">
                    <span class="material-symbols-outlined lb-icon" style="font-size: 3rem; display: block; margin-bottom: 12px;">photo_camera</span>
                    <h3 style="color: var(--lb-ink); margin-bottom: 8px;">{{ $tenant->getSiteContent('gallery_empty_title', 'No Gallery Photos Published Yet') }}</h3>

...
<head>
    ...
    <style>
        .lb-menu-hero-section {
            background: var(--lb-lavender, #f6eefa);
            text-align: center;
            padding: 90px 20px 60px 20px;
            border-bottom: 1px solid rgba(94,44,120,0.08);
        }
        .lb-menu-hero-subtitle {
            text-transform: uppercase;
            font-size: 0.85rem;
            letter-spacing: 3px;
            font-weight: 700;
            color: var(--lb-purple, #8e4fa3);
            margin-bottom: 12px;
            display: block;
        }
        .lb-menu-hero-title {
            ...
            font-size: 3.5rem;
            font-weight: 800;
            color: var(--lb-ink, #3b1f47);
            line-height: 1.15;
            max-width: 800px;
            margin: 0 auto 16px auto;
        }
        .lb-image-menu-wrapper {
            max-width: 900px;
            margin: 40px auto;
            padding: 20px;
            text-align: center;
        }
        .lb-image-menu-wrapper img {
            width: 100%;
            max-width: 850px;
            border-radius: 20px;
            box-shadow: 0 15px 40px rgba(94,44,120,0.15);
            border: 1px solid rgba(94,44,120,0.1);
            cursor: pointer;
        }
        .lb-menu-text-section {
            max-width: 950px;
            ...
            border-radius: 24px;
            border: 1px solid rgba(94,44,120,0.1);
            box-shadow: 0 10px 30px rgba(94,44,120,0.06);
        }
        .lb-menu-text-section h2,
        .lb-menu-text-section h3 {
            font-family: var(--theme-heading-font, 'Playfair Display', serif);
            color: var(--lb-ink, #3b1f47);
            margin-top: 32px;
            margin-bottom: 16px;
            padding-bottom: 10px;
            border-bottom: 2px dashed var(--lb-purple, #8e4fa3);
            font-size: 1.6rem;
            font-weight: 700;
        }
        .lb-menu-text-section h2:first-child,
        .lb-menu-text-section h3:first-child {
            margin-top: 0;
        }
        .lb-menu-text-section p {
            font-size: 1.05rem;
            line-height: 1.8;
            color: var(--lb-ink, #3b1f47);
            margin-bottom: 16px;
        }
        .lb-menu-text-section ul,
        .lb-menu-text-section ol {
            margin: 14px 0 24px 0;
            padding-left: 10px;
            list-style-type: none;
        }
        .lb-menu-text-section li {
            position: relative;
            padding-left: 26px;
            margin-bottom: 10px;
            font-size: 1.02rem;
        ...
        .lb-menu-text-section ul li::before {
            content: "•";
            position: absolute;
            left: 6px;
            top: 0;
...
<div id="menu-page-view">
    <section class="lb-menu-hero-section">
        <span class="lb-menu-hero-subtitle">{{ $tenant->getSiteContent('menu_hero_subtitle', 'OUR OFFERINGS') }}</span>
        <h1 class="lb-menu-hero-title">{{ $tenant->getSiteContent('menu_hero_title', 'Menu & Pricing') }}</h1>
        <p style="font-size:1.05rem; color:var(--lb-ink); opacity:0.85; max-width:650px; margin:0 auto 24px auto;">{{ $tenant->getSiteContent('menu_hero_text', 'Explore our handcrafted baked goods and custom options.') }}</p>
    ...
    @if(!$hasMenuContent)
        ...
        <section style="max-width:700px; margin:70px auto; padding:60px 30px; text-align:center; background:var(--theme-card-bg, #ffffff); border-radius:24px; border:2px dashed var(--lb-purple, #8e4fa3); box-shadow:0 10px 30px rgba(94,44,120,0.06);">
            <div class="material-symbols-outlined lb-icon" style="font-size:3.5rem; margin-bottom:16px;">bakery_dining</div>
            <h2 style="font-family:var(--theme-heading-font); color:var(--lb-ink); font-size:1.8rem; margin-bottom:12px;">{{ $tenant->getSiteContent('menu_empty_title', 'Menu Coming Soon') }}</h2>
    ...
    @else
        ...
        @if(($menuType === 'image' || $menuType === 'both') && $hasImage)
            <section class="lb-image-menu-wrapper">
                <h2 style="font-family:var(--theme-heading-font); color:var(--lb-ink); margin-bottom:12px; font-size:1.8rem;"><span class="material-symbols-outlined" style="vertical-align:-4px;">description</span> Official Bakery Menu</h2>
                @if($isPdf)
                    <div style="background:var(--theme-card-bg, #ffffff); border:1px solid rgba(94,44,120,0.12); border-radius:20px; padding:40px 20px; box-shadow:0 10px 30px rgba(94,44,120,0.05); max-width:700px; margin:20px auto;">
                        <div class="material-symbols-outlined lb-icon" style="font-size:4rem; margin-bottom:12px;">picture_as_pdf</div>
                        <h3 style="font-family:var(--theme-heading-font); color:var(--lb-ink); margin-bottom:8px;">Official Menu PDF</h3>
        ...
        @if($hasProducts && ($menuType === 'text' || $menuType === 'both'))
            <section style="max-width:1100px; margin:40px auto 40px auto; padding:0 20px;">
                <h2 style="font-family:var(--theme-heading-font); color:var(--lb-ink); text-align:center; margin-bottom:25px; font-size:2rem;"><span class="material-symbols-outlined" style="vertical-align:-5px;">cake</span> Featured Catalog &amp; Pricing</h2>
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    @foreach($products as $p)
                        <div class="category-card-exact" style="padding:22px; background:var(--theme-card-bg, #ffffff); border-radius:18px; box-shadow:0 6px 20px rgba(94,44,120,0.08); display:flex; align-items:center; justify-content:space-between; gap:12px;">
                            <div>
                                <h4 style="margin:0 0 4px 0; font-size:1.1rem; color:var(--lb-ink);">{{ $p->name }}</h4>
                                <span style="font-size:0.85rem; color:#777;">Base Price</span>
                            </div>
                            <div style="font-size:1.3rem; font-weight:700; color:var(--lb-purple);">${{ number_format($p->price, 2) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>
        ...
        @if(!empty($customText))
            <section class="lb-menu-text-section">

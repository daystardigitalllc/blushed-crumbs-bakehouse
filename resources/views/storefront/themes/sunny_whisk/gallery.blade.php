...
<div id="gallery-page-view">
    <section class="sw-page-hero">
        <span class="sw-page-hero-kicker">Follow Along</span>
        <h1 class="sw-page-hero-title">{{ $tenant->getSiteContent('gallery_hero_title', 'Our Gallery') }}</h1>
        <p style="font-size:1.05rem; color:var(--sw-ink); max-width:600px; margin:14px auto 0 auto; padding:0 15px;">{{ $tenant->getSiteContent('gallery_hero_text', 'Explore our latest custom creations uploaded directly from our kitchen!') }}</p>
    </section>
    ...
    <section class="gallery-page-section" style="padding-top:0;">
        ...
        <!-- gallery-masonry-grid is kept (not renamed) since app.js's filter
             and add-photo logic selects cards by that class name; the diagonal
             mustard band is layered on via sw-featured-grid. -->
        <div class="gallery-masonry-grid sw-featured-grid" id="public-gallery-grid">
            ...
            @empty
                <div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: #888;">
                    <span class="material-symbols-outlined sw-icon" style="font-size: 3rem; display: block; margin-bottom: 12px;">photo_camera</span>
                    <h3 style="color: var(--sw-ink); margin-bottom: 8px;">{{ $tenant->getSiteContent('gallery_empty_title', 'No Gallery Photos Published Yet') }}</h3>

...
<head>
    ...
    <title>Order Request Received #{{ $order->order_number }}</title>
    <style>
        ...
        .intro-line { font-size: 1.02rem; margin-bottom: 22px; }
        ...
        .next-steps { background: #fff7fa; border: 1px solid #f8c6d7; border-radius: 12px; padding: 16px 18px; margin-top: 20px; font-size: 0.92rem; }
...
<body>
    <div class="email-container">
        <div class="email-header">
            <h1>🌸 {{ $tenant->name ?? 'Our Bakehouse' }}</h1>
            <p>We got your order request!</p>
            <span class="order-badge">Order #{{ $order->order_number }}</span>
        </div>

        <div class="email-body">
            <p class="intro-line">Hi {{ $order->client_name }}, thanks for your order request! We've received it and will follow up shortly with a formal invoice and quote to confirm your date.</p>
            ...
            <div class="next-steps">
                <strong>What happens next?</strong> This is a request, not a confirmed booking yet — we'll reach out with an invoice and deposit link to lock in your date. Have a question in the meantime? Just reply to this email.
            </div>
        </div>

        <div class="footer">
            <p>{{ $tenant->name ?? 'Our Bakehouse' }}{{ $tenant->phone ? ' · ' . $tenant->phone : '' }}</p>
            <p>This confirms your request was received — it is not yet a confirmed booking.</p>
        </div>
    </div>
</body>
</html>
